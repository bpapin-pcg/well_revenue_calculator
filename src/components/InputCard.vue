                    ...
                    <div class="w-0 items-center m-auto p-5 flex-1 bg-white rounded-r-md truncate">
                        <dl>
                            <div class="inline-block w-full">
                                ...
                                <button v-show="editing_avg_oil" @click="editing_avg_oil = false; this.avg_oil_ref=''; main.setInputs(main.getOldInputs)" class="float-right"> <XIcon class="h-5 w-5 text-black" aria-hidden="true" /> </button>
                            </div>
                            <dd>
                    ...
                    <div class="w-0 items-center m-auto p-5 flex-1 bg-white rounded-r-md truncate">
                        <dl>
                            <div class="inline-block w-full">
                                ...
                                <button v-show="editing_latest_oil" @click="editing_latest_oil = false; this.latest_oil_ref=''; main.setInputs(main.getOldInputs)" class="float-right"> <XIcon class="h-5 w-5 text-black" aria-hidden="true" /> </button>
                            </div>
                            <dd>
                    ...
                    <div class="w-0 items-center m-auto p-5 flex-1 bg-white rounded-r-md truncate">
                        <dl>
                            <div class="inline-block w-full">
                                ...
                                <button v-show="editing_non_prod" @click="editing_non_prod = false; this.non_prod_bbls_ref=''; main.setInputs(main.getOldInputs)" class="float-right"> <XIcon class="h-5 w-5 text-black" aria-hidden="true" /> </button>
                            </div>
                            <dd>
                                <input v-show="editing_non_prod" type="number" name="non_prod_bbls" id="non_prod_ref" v-model="non_prod_bbls_ref" @input="onChange" :placeholder="main.getOldInputs.non_prod_bbls.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' BBLs'" class="shadow-sm h-8 pl-3 focus:ring-lightgold focus:border-lightgold block w-full sm:text-sm border-2 border-gray-200 rounded-md" />
                                <span v-show="!editing_non_prod" @dblclick="editing_non_prod=true" class="float-left text-2xl">{{main.getOldInputs.non_prod_bbls.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' BBLs'}}</span>
                            </dd>
                        </dl>
                    </div>
                </div>
                    ...
                    <div class="w-0 items-center m-auto p-5 flex-1 bg-white rounded-r-md truncate">
                        <dl>
                            <div class="inline-block w-full">
                                ...
                                <button v-show="editing_account_value" @click="editing_account_value = false; this.account_value_ref=''; main.setInputs(main.getOldInputs)" class="float-right"> <XIcon class="h-5 w-5 text-black" aria-hidden="true" /> </button>
                            </div>
                            <dd>
                                <input v-show="editing_account_value" type="number" name="acct_val_oil" id="acct_val_ref" v-model="account_value_ref" @input="onChange" :placeholder="'$' + (main.getOldInputs.account_value ? main.commaSeparate(main.getOldInputs.account_value) : main.commaSeparate(main.getAccount.total_landholding_value))" class="shadow-sm h-8 pl-3 focus:ring-gray focus:border-gray block w-full sm:text-sm border-2 border-gray-200 rounded-md" />
